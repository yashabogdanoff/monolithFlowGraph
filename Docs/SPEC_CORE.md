...
Counts below were re-verified against the live `monolith_discover()` registry on 2026-05-03 (v0.14.10 [Unreleased] audit). Where a module's spec previously claimed a different number, those per-module SPEC files were corrected. Reflected: Phase J F8 deltas (`editor` +2, `gas` +1, `ai` +2, `audio` +1); Phase A–L MonolithUI deltas (+21 net: hoisted Design Import 5, Animation v2 core+events 5, EffectSurface 10, Spec Builder/Schema/Serializer 3, Type Registry diagnostic 1, Style Service diagnostic 1 conditional, plus 2 v1 actions tagged `[DEPRECATED]` but still registered); v0.14.8 `editor` +2 (`run_automation_tests`, `list_automation_tests` — PR #48 by @MaxenceEpitech); v0.14.9 `editor` +2 (`run_python`, `load_level` — Issue #50, ported from @JCSopko's fork) and `animation` +1 (`copy_bone_pose_between_sequences` — PR #51 by @MaxenceEpitech); v0.14.10 [Unreleased] `audio` +12 MetaSound document introspection actions (PR #18 by @alakangas, refactored into `audio_query` namespace) plus `FMetaSoundIndexer` deep indexer added to MonolithIndex (also PR #18 by @alakangas). v0.14.10 [Unreleased] PR #54 by @MaxenceEpitech: animation +1 (`list_bone_tracks`), editor +3 (`start_pie`, `stop_pie`, `run_console_command`), plus rewrite of `get_bone_track_keys` to non-deprecated API. v0.14.10 [Unreleased] PR #55 by @MaxenceEpitech: animation +2 (`get_skeleton_preview_attached_assets`, `get_bone_ref_pose`). v0.14.10 [Unreleased] PR #56 by @MaxenceEpitech: animation +3 (`get_compatible_skeletons`, `add_compatible_skeleton`, `remove_compatible_skeleton`). v0.14.10 [Unreleased] PR #57 by @MaxenceEpitech: blueprint.`get_cdo_properties` gains 3 optional filter params (`owner_class_filter` / `name_pattern` / `exclude_categories`) — schema-only change, no action-count delta.
...
| **Total** | | **1299** registrations across 16 namespaces + 4 `monolith_*` meta = **1295 in-tree namespace actions** active by default (45 town-gen experimental disabled). With town gen registered: 1344. Without `WITH_COMMONUI`: 1244 (and 1289 with town gen); without `WITH_GBA`: 1164 (1209 w/ town gen); without both: 1113 (1158 w/ town gen). Without `WITH_METASOUND`: 1283 (the v0.14.10 12 introspection actions become inert) | The `ui` namespace double-counts 4 aliased GAS actions; **distinct** action handlers = **1291** in default-active configuration, **1340** with town gen registered |
